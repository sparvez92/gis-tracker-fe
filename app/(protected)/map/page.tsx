...
import SearchFilterHeader from '@/components/custom/Filters';
...
function UserLocationMarker({ map, coords }: { map: L.Map | null; coords: L.LatLng | null }) {
  ...
  return (
    <Marker
      position={coords}
      icon={MapPin as unknown as L.Icon<L.IconOptions>}
    >
    </Marker>
  );
}
...
/* ---------- Main Component ---------- */
export default function MapWithGoogleSearchHybrid() {
  ...
  useEffect(() => {
    if (!searchInputRef.current) return;
    if ((window as any).google && (window as any).google.maps) initAutocomplete();
    // eslint-disable-next-line react-hooks/exhaustive-deps
